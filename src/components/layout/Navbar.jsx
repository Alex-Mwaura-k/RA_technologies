...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-blue-50/90 backdrop-blur-md border-b border-blue-100 transition-all">
      <div className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8">
        {/* REDUCED HEIGHT: Changed from h-20 to h-16 (64px tall) */}
        <div className="flex items-center justify-between h-16">
          
          {/* Left: Brand Logo */}
          <div className="flex-1 flex items-center justify-start group">
            <Link to="/" className="flex items-center gap-2">
              <div className="w-9 h-9 bg-[var(--deep-black)] text-white font-extrabold flex items-center justify-center rounded-lg text-sm tracking-widest shadow-sm group-hover:bg-[var(--tech-blue)] transition-colors">
                RA
              </div>
              <span className="text-xl font-extrabold text-[var(--deep-black)] tracking-tight hidden sm:block">
          ...
          <div className="hidden md:flex flex-1 items-center justify-center gap-8">
            ...
            <div className="relative group py-5">
              ...
              {/* Desktop Dropdown 1 */}
              <div className="absolute top-[85%] left-1/2 -translate-x-1/2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl p-2 hidden group-hover:block animate-fade-in z-50">
            ...
            <div className="relative group py-5">
              ...
              {/* Desktop Dropdown 2 */}
              <div className="absolute top-[85%] left-1/2 -translate-x-1/2 w-72 bg-white border border-slate-200 rounded-xl shadow-xl p-2 hidden group-hover:block animate-fade-in z-50">
            ...
            {/* Added Blog Link */}
            <Link to="/blog" className="text-sm font-semibold text-slate-600 hover:text-[var(--tech-blue)] transition-colors py-5">
              Blog
            </Link>

            {/* Smart Links (detects if on homepage) */}
            <a href={isHome ? '#about' : '/#about'} className="text-sm font-semibold text-slate-600 hover:text-[var(--tech-blue)] transition-colors py-5">
              About
            </a>
          </div>
          ...
          <div className="flex-1 flex items-center justify-end gap-3">
            ...
            <div className="md:hidden flex items-center relative z-50">
              <button
                ...
                className="inline-flex items-center justify-center p-2 -mr-2 rounded-md text-slate-500 hover:text-[var(--deep-black)] hover:bg-slate-100 focus:outline-none transition-colors"
              >
      ...
      {/* 1. Backdrop Blur Overlay - Updated top offset to top-16 */}
      <div 
        className={`md:hidden fixed inset-0 top-16 bg-slate-900/20 backdrop-blur-sm transition-opacity duration-300 z-40 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsOpen(false)}
      ></div>

      {/* 2. Slide-in Side Drawer - Updated top offset to top-16 and height calc to 4rem */}
      <div 
        className={`md:hidden fixed top-16 right-0 w-72 h-[calc(100vh-4rem)] bg-white border-l border-slate-200 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6">
          ...
          <div>
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isMobileSaaSOpen ? 'max-h-64 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
              <div className="flex flex-col space-y-1 border-l-2 border-slate-100 ml-2 pl-4">
        ...
        <div className="p-5 border-t border-slate-200 bg-slate-50 space-y-3 pb-8">
          <Link to="/login" onClick={() => setIsOpen(false)} className="flex items-center justify-center w-full bg-white border border-slate-300 text-slate-700 px-4 py-3 rounded-lg font-bold text-sm transition-colors shadow-sm">
            Login
          </Link>
          <Link to="/signup" onClick={() => setIsOpen(false)} className="flex items-center justify-center w-full bg-[var(--deep-black)] text-white px-4 py-3 rounded-lg font-bold text-sm shadow-md transition-colors">
            Sign Up
          </Link>
        </div>
      </div>
